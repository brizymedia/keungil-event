...
「무대 구조물」 고르기()의 값이다. 이름을 바꾸지 말 것 — 형님이 쓰는 현장 용어다.

| 값 | 이름 | 생김새 |
|---|---|---|
|  | 외줄 트러스 | 타워 두 본 사이에 가로 트러스 **한 줄**. 조명만 건다. 무대는 철골 · 차량 무대로 따로 선다 |
|  | 사각 트러스 (골대형) | 사각 단면 트러스 기둥 2본 + 가로대 1줄 |
|  | 6발 트러스 | 육각 단면 트러스로 세운 골대형. 가로대가 위 · 아래 2줄 |
|  | 6발 트러스 + 루프 | 위에 지붕까지. 대형 축제 무대 |
|  | 레이허 구조물 | 트러스 없이 비계(레이허)만으로 양쪽 타워를 쌓고 조명 바를 단다 |

 는 옛 이름으로  로 읽는다(예전에 공유한 링크 호환). LED 뒤를 받치는 「LED 뒤 지지 틀」()은 이것과 다른 별개 스위치다.
