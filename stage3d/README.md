...
| `hex` | 6발 트러스 | **기둥 발이 6개** — 사각 트러스(발 4개)에 스피커를 매달려고 양옆으로 한 발씩 더 낸 것. **스피커 좌우 끝에 기둥이 서고 스피커는 그 두 기둥 사이에 리깅**된다(바닥 적재 아님, 서브우퍼만 바닥). 가로대는 맨 위 한 줄만(아래에 더 걸면 LED 를 가로지른다) |
| `hexroof` | 6발 트러스 + 루프 | 6발에 지붕까지. 지붕은 **가운데 마룻대를 앞뒤로 걸고 좌 · 우로 갈라져** 내려온다(앞뒤는 트여 있다). 대형 축제 무대 |
| `layher` | 레이허 구조물 | 트러스 없이 비계(레이허)만으로 양쪽 타워를 쌓고 조명 바를 단다 |

**치수 (형님 확인)** — 기둥도 보도 **600각 트러스**(600×600mm). 6발은 그 600각에 스피커를 매달 발을 양옆으로 하나씩 더 낸 것이고, 스피커가 걸리는 두 기둥 사이는 **3m**.
몽골텐트는 **5×5m 사각**에 흰 천 원뿔(피라미드) 지붕 — 처마 2.2m · 꼭대기 3.4m. 게르(원형)가 아니다.
