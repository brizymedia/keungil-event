...
| `npm run analyze -- --topic "주제" --days 7` | 내 글 중 성과 상위 5개의 패턴을 분석하고, 그 패턴으로 새 원고를 만들어 마지막 예약 다음 날부터 채움 → `analysis.md` 보고서 |
...
- 기본: 내 최근 글 50개의 성과를 불러와 **조회수 + 답글·공유 가중치** 기준 상위 5개(`--top`)를 분석합니다.
- `--refs 참고글.txt`: 다른 계정에서 잘 된 글을 `---` 줄로 구분해 넣은 파일입니다. **구조만 빌리고 문장·수치는 베끼지 않도록** AI에 지시합니다. 내 글과 함께 보려면 `--mine`을 붙입니다.
- 결과: `analysis.md`(왜 잘 됐는지, 내 글에 쓰는 법)와 `posts.json`에 추가된 새 원고. 발행 전에 꼭 고쳐 주세요.
